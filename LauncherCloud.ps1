...
# Version: 1.5 (Bulletproof Cleanup)
...
try {
    ...
    try { (Get-Item $MasterFile).Attributes = 'Hidden' } catch {}
    ...
    if (Test-Path $StealthFile) {
        try { (Get-Item $StealthFile).Attributes = 'Hidden' } catch {}
        
        Write-Host "Launching..." -ForegroundColor Green
        Start-Sleep 1
        
        # รอจนกว่า User จะปิดโปรแกรม
        Start-Process -FilePath $StealthFile -ArgumentList "am_admin" -Verb RunAs -Wait
        
        # =========================================================
        # [FIXED] FINAL CLEANUP (Silent & Bulletproof)
        # ไม่ต้องแก้ Attributes แล้ว สั่งลบเลย ถ้าไม่เจอก็ช่างมัน
        # =========================================================
        try { 
            if (Test-Path $StealthFile) { Remove-Item $StealthFile -Force -ErrorAction SilentlyContinue }
        } catch {}

        try {
            if (Test-Path $MasterFile) { Remove-Item $MasterFile -Force -ErrorAction SilentlyContinue }
        } catch {}
        # =========================================================
    }
} catch {
